...
boot_alloc(uint32_t n)
{
	...
	char *temp;	// virtual address of next byte of free memory
...
page_init(void)
{
	...
	//
	//
	//  |    base memory   |  io hole |     extern memory     |
	//  | zero page | free | reserved | boot allocated | free |
	//
	//
	// Change the code to reflect this.
	// NB: DO NOT actually touch the physical memory corresponding to
	// free pages!
	size_t i, allocated_pgnum, iohole_pgnum, free_pgnum = 0;

	allocated_pgnum = PGNUM((size_t)boot_alloc(0) - KERNBASE);
	iohole_pgnum = PGNUM(EXTPHYSMEM - IOPHYSMEM);
...
{
	pte_t *pt_entry;
	int i, npage = ROUNDUP(size, PGSIZE) / PGSIZE;

	for (i = 0; i < npage; i++) {
		pt_entry = pgdir_walk(pgdir, (void *)va, 1);
		*pt_entry = (PTE_ADDR(pa) | PTE_P | perm);
