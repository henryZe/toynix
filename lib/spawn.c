...
init_stack(envid_t child, const char **argv, uintptr_t *init_esp)
{
	...
	ret = sys_page_alloc(0, (void *)UTEMP, PTE_W);
	if (ret < 0)
		return ret;
	...
	argv_store[argc] = 0;	/* NULL as the terminal */
	...
	*init_esp = UTEMP2USTACK(&argv_store[-2]);
	...
	ret = sys_page_map(0, UTEMP, child, (void *)(USTACKTOP - PGSIZE), PTE_W);
	if (ret < 0)
		goto error;
		...
		int fd, size_t filesz, off_t file_offset, int perm)
{
	...
	for (i = 0; i < memsz; i += PGSIZE) {
		if (i >= filesz) {
			...
			ret = sys_page_map(0, UTEMP, child, (void *)(va + i), perm);
			if (ret < 0)
				panic("spawn: sys_page_map data: %e", ret);
...
spawn(const char *prog, const char **argv)
{
	...
	ret = readn(fd, elf_buf, sizeof(elf_buf));
	if (ret != sizeof(elf_buf) || elf->e_magic != ELF_MAGIC) {
		close(fd);
