...
start_timer(struct timer_thread *t, void (*func)(void), const char *name, int msec)
{
	...
	ret = thread_create(NULL, name, &net_timer, (uint32_t)t);
	if (ret < 0)
...
serve_thread(uint32_t a)
{
	...
	if (args->reqno != NSREQ_INPUT)
		ipc_send(args->whom, r, NULL, 0);

	put_buffer(args->req);
	sys_page_unmap(0, args->req);
	free(args);
}
...
serve(void)
{
	...
	while (1) {
		...
		perm = 0;
		va = get_buffer();	/* get request address space */
		req_no = ipc_recv(&whom, (void *)va, &perm);
		if (debug)
			printf("ns req %d from %08x\n", req_no, whom);

		// first take care of requests that do not contain an argument page
		if (req_no == NSREQ_TIMER) {
			process_timer(whom);
			put_buffer(va);
			/* no need to unmap because timer not maps va */
			continue;
		}
